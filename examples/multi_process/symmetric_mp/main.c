...
lcore_main(void *arg __rte_unused)
{
	...
	for (;;) {
		...
		{			
			for (i = 0; i < 4; i++) {
				int rx_c = rte_eth_rx_burst(port_id,
							i, buf, 32);
				if (rx_c == 0)
					continue;
				pstats[port_id].rx += rx_c;
				const uint16_t tx_c = rte_eth_tx_burst(port_id^1, q_id, buf, rx_c);
				pstats[port_id^1].tx += tx_c;
				if (tx_c != rx_c) {
					pstats[port_id^1].drop += (rx_c - tx_c);
					for (i = tx_c; i < rx_c; i++)
						rte_pktmbuf_free(buf[i]);
				}
			}
