...
static uint64_t test_hash(void *key,
...
static struct rte_table_ops lru_ops={
...
void test_table(){
    ...
    void *table=lru_ops.f_create(&p, 0, 1);
    ...
    status = lru_ops.f_add(table, &key, &entry, &key_found, &entry_ptr);
	if (status != 0)
		printf("add status: %d\n", status);



    status = lru_ops.f_free(table);
    if(status < 0)
